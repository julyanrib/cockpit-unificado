...
const PROPS_OBRIGATORIAS_POR_ETAPA = {
  ...
  /* AG. PAGAMENTO PARA DE PEDIR amount E mrr (02/09/26). Ela pedia os TRÊS campos de
     ...
     Agora existe UM campo digitado (valor_de_mrr) e os outros dois são derivados dele
     mais o período — ver derivarDinheiro(). Três lugares para digitar o mesmo número são
     três lugares para errar. */
  '1395880473': ['dealname', 'email', 'cnpj_cpf', 'celular', 'cep', 'numero',
    'pacote_contratado', 'adicional', 'tipo_de_pagamento', 'periodo_contratado',
    'valor_de_mrr', 'deseja_criar_perfil_no_asaas_', 'qual_maior_desafio_',
...
function derivarDinheiro(finais, propriedades) {
  ...
  const saida = { ...propriedades, mrr: String(mensal) };
  const meses = MESES_DO_PERIODO[String(finais.periodo_contratado || '').trim()];
  if (meses) saida.amount = String(Number((mensal * meses).toFixed(2)));
  return saida;
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    /* DINHEIRO DERIVADO, DEPOIS DA VALIDACAO E ANTES DA ESCRITA. Depois porque a
       ...
       e tudo tem que ir junto. O valor derivado sobrescreve o que o cliente mandou em
       amount/mrr — ver derivarDinheiro(). */
