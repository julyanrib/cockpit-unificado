...
function pedacosDoEndereco(endereco) {
  ...
  /* A UF ancora tudo: "ES, 29055-145, Brasil". Procuro DE TRAS PARA A FRENTE porque
     complemento ("Loja 336", "Ground floor") aparece no meio e empurra os indices.
     DE TRAS PARA A FRENTE e defensivo, e nao provado: nos 209 enderecos desta carga o
     primeiro trecho com cara de UF ja era o certo. Fica assim porque o complemento e
     livre e um dia vai ter duas maiusculas e virgula no meio — mas nao afirmo que a
     guarda cobre isso, porque nao cobre. */
  let k = -1;
  for (let i = partes.length - 1; i >= 1; i--) {
    if (/^[A-Z]{2}(,|$)/.test(partes[i])) { k = i; break; }
  }
  if (k < 1) return vazio;

  const estado = partes[k].slice(0, 2).toUpperCase();
  const anterior = partes[k - 1].split(',').map(function (x) { return x.trim(); }).filter(Boolean);
  /* EXATAMENTE duas partes = "Bairro, Cidade". Tres ou mais e logradouro solto antes da
     cidade, e ai bairro e null de proposito — chutar e pior que nao ter. */
  const cidade = anterior.length ? anterior[anterior.length - 1] : null;
  const bairro = anterior.length === 2 ? anterior[0] : null;
